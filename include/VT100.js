	...
	for (i = 0; i < stuff.length; ++i) {
		...
		switch (this.esc_state_) {
			...
			switch (ch) {
				...
				break;
			    case 'd':
				// <n>` - set column (absolute)
                                this.move(this.csi_parms_[0] - 1, this.col_);
				break;
			    case '`':
				// <n>` - set column (absolute)
                                this.move(this.row_,this.csi_parms_[0] - 1);
				break;
			    default:
			    this.warn("write:: unknown command: " + ch.charCodeAt(0).toString(16));
				this.csi_parms_ = [];
				break;
			}
			break;
